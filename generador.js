...
const handleKeyDownForNumericInputs = (e) => {
    ...
    if (e.keyCode === MINUS_KEYCODE || e.keyCode === PLUS_KEYCODE || e.keyCode === EULER_KEYCODE){
        e.preventDefault();
            ...
            // Si el valor del input está vacío, no hagas nada
        } else if (e.target.value < min || e.target.value > max) {
            e.target.value = e.target.value <= min ? min : max;
        }
};
